...
if Settings.Aura.PushAura then
    task.spawn(function()
        FTAP.SetNetworkOwner(v)
        ...
        AuraHelper.Velocity(
            v,
            direction * 4000 + Vector3.new(0, 800, 0)
        )
    end)
end
...
-- タブ一括作成（指定された順序）
-- ============================================
local GrabTab = Window:MakeTab({Name = "Combat",Icon = "rbxassetid://7733955740",PremiumOnly = false})
local AntiTab = Window:MakeTab({Name = "invisibility",Icon = "rbxassetid://7734056411",PremiumOnly = false})
local PlayerTab = Window:MakeTab({Name = "Player",Icon = "rbxassetid://7733956134",PremiumOnly = false})
local BlobmanBetaTab = Window:MakeTab({Name = "Blobman",Icon = "rbxassetid://7733916988",PremiumOnly = false})
local SnowTab = Window:MakeTab({Name = "Snowball",Icon = "rbxassetid://7733955740",PremiumOnly = false})
...
local TeleportTab = Window:MakeTab({Name = "Teleport",Icon = "rbxassetid://7733955740",PremiumOnly = false})
...
-- グラブタブ
-- ============================================
GrabTab:AddSection({ Name = "Grab Strength" })
...
GrabTab:AddSection({ Name = "Grab Settings" })
...
-- =========================
local auraReach = 30
local flingStrength = 30000
local upStrength = 200

-- =========================
-- 状態
-- =========================
local killGrabActive = false
local spinGrabActive = false
local skyGrabActive = false
...
-- =========================
RunService.Heartbeat:Connect(function()
    if not flingAuraEnabled and not chaosAuraEnabled then return end
    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= localPlayer and plr.Character then
            ...
            if targetRoot then
                ...
                if dist <= auraReach then
                    -- 掴みパーツ作成
                    local grabPart = Instance.new("Part")
                    grabPart.Size = Vector3.new(0.2,0.2,0.2)
                    grabPart.Transparency = 1
                    grabPart.Anchored = false
                    grabPart.CanCollide = false
                    grabPart.CFrame = root.CFrame
                    grabPart.Parent = Workspace
...
BlobmanBetaTab:AddToggle({
    ...
    Default = false,
    Callback = function(v)
        Settings.BlobmanBeta.isLooping = v

        if v then
            -- 非同期でループ開始
            task.spawn(function()
                while Settings.BlobmanBeta.isLooping do
                    if Settings.BlobmanBeta.isExecuting then
                        task.wait(1)
                    else
                        Settings.BlobmanBeta.isExecuting = true
                        ...
                        local success = (function()
                            ...
                            for _, plr in ipairs(Players:GetPlayers()) do
                                ...
                                if not root then continue end
                                local playerY = root.Position.Y
                                if playerY < -25 then continue end
                                local shouldIgnore = false
                                for _, ignoreHeight in ipairs(Settings.BlobmanBeta.ignoreHeights) do
                                    if math.abs(playerY - ignoreHeight) < 0.1 then
                                        shouldIgnore = true
                            ...
                            BlobmanBetaFeature.destroyBlobman()
                            OrionLib:MakeNotification({Name = "Complete", Content = "Kick All executed", Time = 3})
                            return true
                        end)()
                        
                        Settings.BlobmanBeta.isExecuting = false
                        task.wait(1) -- 次ループまで少し待機
                    end
                end
            end)
        end
...
local function EnableSpeed()
    ...
    speedBV.MaxForce = Vector3.new(1e5, 0, 1e5) -- horizontal only
...
RunService.RenderStepped:Connect(function()
    ...
    if direction.Magnitude > 0 then
        speedBV.Velocity = Vector3.new(direction.X * SpeedValue, 0, direction.Z * SpeedValue)
    else
        speedBV.Velocity = Vector3.zero
    end
end)
...
local jumpValue = 50
local humConn
local MULTIPLIER = 3.2 -- jump multiplier
...
-- Reapply jump on respawn
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.2)
    ApplyJump()
end)

-- Reapply jump on jump request
...
-- ===== UI =====
local MovementSection = PlayerTab:AddSection({ Name = "Player Movement" })
...
MovementSection:AddSlider({
    ...
    CurrentValue = 16,
    Callback = function(value)
        SpeedValue = value * 5 -- internal multiplier (max 500)
    end
})

-- Jump Section
local JumpSection = PlayerTab:AddSection({ Name = "Jump Settings" })
...
-- Noclip Section
local NoclipSection = PlayerTab:AddSection({ Name = "Noclip" })
...
local upDownAuraActive = false
local auraRange = 50

local upDownSpeed = 200   -- スピード（スライダー）
local upDownHeight = 50   -- 高さ（スライダー）
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= localPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if dist <= auraRange then
                ...
                -- 上下ランダム（高さ範囲を制御）
                local yPower = math.random(-upDownHeight, upDownHeight)

                bv.Velocity = Vector3.new(
                    0,
                    yPower * (upDownSpeed / 100), -- スピード倍率
                    0
                )
...
local function startUpDownAura()
    if upDownConn then return end
    upDownConn = RunService.Heartbeat:Connect(function()
        if upDownAuraActive then
            grabNearbyPlayers()
        end
    end)
end
...
local spinRadius = 6     -- 広がり（スライダー）
local spinSpeed = 10     -- 回転スピード（スライダー）
local spinHeight = 2     -- 高さ（スライダー）
...
local function getPlayersInRange(range)
    local t = {}
    local myChar = localPlayer.Character
    if not myChar or not myChar:FindFirstChild("HumanoidRootPart") then return t end
    local myRoot = myChar.HumanoidRootPart
...
-- 回転処理
local function spinAuraLoop(dt)
    local myChar = localPlayer.Character
    ...
    local targets = getPlayersInRange(spinRange)
    local count = #targets
    if count == 0 then return end

    for i, hrp in ipairs(targets) do
        -- 均等に配置（円形）
        local offsetAngle = angle + (i / count) * math.pi * 2
        ...
        -- 掴む
        pcall(function()
            SetNetworkOwner:FireServer(hrp, targetCF)
        end)

        -- 吸い寄せ＋回転
        local bv = Instance.new("BodyVelocity")
        bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
        bv.Velocity = (targetPos - hrp.Position) * 12
        bv.Parent = hrp
...
-- トグル
local function toggleSpinAura(state)
    spinActive = state
...
local Toys = RS:WaitForChild("MenuToys")
local SpawnToyRemote = Toys:WaitForChild("SpawnToyRemoteFunction")
...
local function updateDropdown(dd)
    if dd then
        dd:Refresh(getAllPlayers(), true)
    end
end

Players.PlayerAdded:Connect(function() updateDropdown(SnowballData.DD) end)
Players.PlayerRemoving:Connect(function() updateDropdown(SnowballData.DD) end)

-- ===== 雪玉生成 =====
local function spawnSnowball(pos)
    pcall(function()
        SpawnToyRemote:InvokeServer("BallSnowball", CFrame.new(pos), Vector3.zero)
    end)
end
...
local function startTP()
    ...
    tpConn = RunService.Heartbeat:Connect(function()
        if not SnowballData.Teleport then return end
        if not SnowballData.Target then return end

        local char = SnowballData.Target.Character
        if not char then return end
        ...
        for _, toy in ipairs(folder:GetChildren()) do
            if toy.Name == "BallSnowball" then
                local part = toy:FindFirstChildWhichIsA("BasePart", true)
                if part then
                    part.CFrame = hrp.CFrame -- 位置＋向き完全一致
                end
            end
        end
    end)
...
local function startSpawn()
    ...
    spawnConn = RunService.Heartbeat:Connect(function()
        if not SnowballData.AutoSpawn then return end
        ...
        spawnSnowball(hrp.Position + Vector3.new(0,2,0))
    end)
end
...
SnowballData.DD = SnowTab:AddDropdown({
    Name = "Select Player",
    Options = getAllPlayers(),
    Default = nil,
    Callback = function(v)
        SnowballData.Target = Players:FindFirstChild(v)
    end
})

SnowTab:AddToggle({
    Name = "Snowball TP Hit",
    Default = false,
    Callback = function(v)
        SnowballData.Teleport = v
...
SnowTab:AddToggle({
    ...
    Default = false,
    Callback = function(v)
        SnowballData.AutoSpawn = v
        if v then startSpawn() else stopSpawn() end
    end
})

-- =========================
-- Teleport Tab / Map Section
-- =========================
local Players = game:GetService("Players")
local Workspace = game:GetService("Workspace")
...
-- Players Section
-- =========================
local PlayersSection = TeleportTab:AddSection({ Name = "Players" })
...
PlayersSection:AddButton({
    ...
    Callback = function()
        ...
        if targetChar and targetChar:FindFirstChild("HumanoidRootPart") and myChar and myChar:FindFirstChild("HumanoidRootPart") then
            ...
            local behindPos = targetHRP.Position - targetHRP.CFrame.LookVector * 5 -- 背後 5スタッド
            LP.Character.HumanoidRootPart.CFrame = CFrame.new(behindPos, targetHRP.Position)
        end
    end
})

local MapSection = TeleportTab:AddSection({ Name = "Map" })
...
local teleportLocations = {
    {Name = "紫の家", Pos = Vector3.new(255, -8, 449)},
    {Name = "緑の家", Pos = Vector3.new(-534, -8, 93)},
    {Name = "青の家", Pos = Vector3.new(512, 82, -343)},
    {Name = "オレンジの家", Pos = Vector3.new(548, 122, -73)},
    {Name = "赤の家", Pos = Vector3.new(-493, -8, -165)},
    {Name = "リス地", Pos = Vector3.new(0, -7, 0)},
    {Name = "毒井戸", Pos = Vector3.new(106, -25, 279)},
    {Name = "スロット裏洞窟", Pos = Vector3.new(-34, -7, -299)},
    {Name = "雪山", Pos = Vector3.new(-414, 231, 480)},
    {Name = "工場", Pos = Vector3.new(138, 365, 346)},
    {Name = "Safespot", Pos = Vector3.new(1e4, 1e4+10, 1e4)}
}
    ...
    rainbowConn = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        for i, part in ipairs(mapParts) do
            if part and part.Parent then
                -- パーツごとにズラして「混ざる感じ」
                local offset = (i % 50) / 50
                local color = Color3.fromHSV((baseHue + offset) % 1, 1, 1)

                part.Color = part.Color:Lerp(color, 0.2) -- なめらか変化
                part.Material = Enum.Material.Neon     -- 光る感じ
            end
        end
...
local function AutoGrabLoop()
    task.spawn(function()
        while Settings.PvP.AutoGrabEnabled do
            ...
            for _, targetPlayer in pairs(game.Players:GetPlayers()) do
                if targetPlayer ~= player and targetPlayer.Character then
                    ...
                    -- 👇 Head優先
                    local targetPart
                    if Settings.PvP.HeadshotOnly then
                    ...
                    if targetPart then
                        -- 👇 掴み処理（ここ重要）
                        FTAP.SetNetworkOwner(targetPart)

                        -- プレイヤーの近くに引き寄せる
                        local myRoot = character:FindFirstChild("HumanoidRootPart")
                        if myRoot then
                            targetPart.CFrame = myRoot.CFrame * CFrame.new(0, 0, -3)
                        end
                    end
                end
            end
...
MiscTab:AddSection({ Name = "ゲロかけオール" })
...
local function getTargetPart(player)
    ...
    return getAimPart(player.Character)
end
